...
abstract class BaseFragment : Fragment(R.layout.fragment_sample) {
    ...
    val fittedImageView: FittedImageView
        get() = requireView().findViewById(R.id.iv_top)

    val fittedTextView: ImageView
        get() = requireView().findViewById(R.id.iv_bottom)

    val primaryColor = Color.RED
    val borderColor by lazy { ContextCompat.getColor(requireContext(), R.color.colorPrimary) }
